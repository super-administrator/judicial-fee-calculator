...
def calc_non_property_case(
    case_type: NonPropertyType,
    amount: float = 0.0,
    base_fee: Optional[float] = None,
) -> float:
    if case_type == "离婚无财产案件":
        # 基本 150；涉财部分超20万按 0.5%
        base = 200.0 if base_fee is None else base_fee
        if amount <= 200_000:
            return base
        return base + (amount - 200_000) * 0.005

    if case_type == "人格权侵权案件":
        # 基本 300；>5万至10万部分按1%；>10万部分按0.5%
        base = 100.0 if base_fee is None else base_fee
        if amount <= 50_000:
            return base
        elif amount <= 100_000:
            return base + (amount - 50_000) * 0.01
        else:
            return base + 500.0 + (amount - 100_000) * 0.005
